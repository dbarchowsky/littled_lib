...
class SerializedContentTest extends TestCase
{
	...
	public static function setUpBeforeClass() : void
	{
		...
		$query = "CRE"."ATE PROCEDURE `testListSelect`() ".
			"BEGIN ".
			"SEL"."ECT * from `".SerializedContentTitleTestHarness::TABLE_NAME()."` ".
			"ORDER BY `title`; ".
			"END;";
		$c->query($query);

		$query = "INS"."ERT INTO `".SerializedContentTitleTestHarness::TABLE_NAME()."` (`title`,`vc_col`,`int_col`) VALUES ('test one','foo',67);".
			"INS"."ERT INTO `".SerializedContentTitleTestHarness::TABLE_NAME()."` (`title`,`vc_col`,`int_col`) VALUES ('test two','bar',860);".
			"INS"."ERT INTO `".SerializedContentTitleTestHarness::TABLE_NAME()."` (`title`,`vc_col`,`int_col`) VALUES ('test three','biz',1032);".
			"INS"."ERT INTO `".SerializedContentTitleTestHarness::TABLE_NAME()."` (`title`,`vc_col`,`int_col`) VALUES ('test four','bash',94);";
		$c->query($query);
	}
	...
	public static function tearDownAfterClass(): void
	{
		...
		$query = "DROP PROCEDURE IF EXISTS `testListSelect`;";
		$c->query($query);
		$query = 'D'.'ROP TABLE `'.SerializedContentChild::TABLE_NAME().'`';
		$c->query($query);
		$query = 'D'.'ROP TABLE `'.SerializedContentNameTestHarness::TABLE_NAME().'`';
		$c->query($query);
		$query = 'D'.'ROP TABLE `'.SerializedContentTitleTestHarness::TABLE_NAME().'`';
		$c->query($query);
		$query = 'D'.'ROP TABLE `'.SerializedContentNonDefaultColumn::TABLE_NAME().'`';
		$c->query($query);
	}
	...
	public function testGetTypeName()
	{
		...
		$query = "SEL"."ECT MAX(`id`) AS `insert_id` FROM `".SerializedContentTitleTestHarness::TABLE_NAME()."`";
		$data = $this->conn->fetchRecords($query);
		$insert_id = $data[0]->insert_id;

		$result = $obj->getTypeName(SerializedContentTitleTestHarness::TABLE_NAME(), $insert_id);
	...
	public function testSave()
	{
		...
		$query = "SEL"."ECT MAX(`id`) as last_insert_id FROM `".SerializedContentChild::TABLE_NAME()."`";
		$data = $this->conn->fetchRecords($query);

		$this->assertEquals($data[0]->last_insert_id, $src->id->value);
		$this->assertEquals('Once upon a time', $src->vc_col2->value);

		$data = $this->conn->fetchRecords("SEL"."ECT * FROM `".SerializedContentChild::TABLE_NAME()."` WHERE `id` = {$src->id->value}");
	...
	public function testSaveDefaultValues()
	{
		...
		$this->conn->fetchRecords("SEL"."ECT * FROM `".SerializedContentChild::TABLE_NAME()."` WHERE `id` = {$obj->id->value}");
	...
	public function testSaveNonDefaultColumns()
	{
		...
		$query = "SEL"."ECT * FROM `".SerializedContentNonDefaultColumn::TABLE_NAME()."` WHERE `id` = {$o1->id->value}";
		$data = $this->conn->fetchRecords($query);
    ...
    public function testSaveNullValues()
    {
	    ...
	    $data = $this->conn->fetchRecords("SEL"."ECT * FROM `".SerializedContentChild::TABLE_NAME()."` WHERE `id` = {$obj->id->value}");
