(function ($) {
    ...
    let settings = {
        allowSorting: true,
        editThumbnailURL: scripts_path+'images/upload_image.php',
        linkThumbnailURL: scripts_path+'images/set_thumbnail.php',
        unlinkThumbnailURL: scripts_path+'images/unlink_thumbnail.php',
        deleteThumbnailURL: scripts_path+'images/del_image.php',
        albumListingsURL: scripts_path+'images/album_listings.php',
        galleryListingsURL: scripts_path+'images/gallery_listings.php',
        ...
        edit: {
            ...
            class_key: 'class',
            csrf_key: 'csrf',
            validate_slug_uri: scripts_path+'utils/validate_slug.php'
        },
        keys: {
            keyword_filter: 'flkw',
            record_id: 'id',
        ...
        previews: {
            ...
            followCursor: true,
            content: function() {
                let $e = $('.tooltip-content', $(this).parent().parent());
                if ($e.length < 1) {
    ...
    let methods = {
        ...
        retrieveGallery: function(options) {
            ...
            return this.each(function() {
                ...
                let $e = $(this);
                let pid = $e.data(lclSettings.keys.parent_id);
                let tid = $e.data(lclSettings.keys.content_type_id);
                if (!pid) {
                ...
                let fd = $.extend({}, lclSettings.galleryFilters, {pid: pid, tid: tid});
         ...
         * @param {Event} evt
         * @param {?object} options Optional collection of settings that will override the library's default settings.
         */
        toggleGallery: function( evt, options ) {
            ...
            let id = $(this).data('id');
            let icons = $(this).button('option', 'icons');

            let $p = $('tr.gallery-row[data-id='+id+']');
            if ($p.length < 1) {
            ...
            if (icons.primary === 'ui-icon-expand') {
                ...
                if ($g.length < 1) {
                    ...
                    let $l = $(lclSettings.dom.gallery_container, $p);
        ...
        animateGalleryToggle: function( options ) {
            ...
            return this.each(function() {
                let id = $(this).data('id');
                let $btn = $('#toggle-'+id+'-btn');
                let icons = $btn.button('option', 'icons');
                if (icons.primary==='ui-icon-expand') {
                    /* expanding */
                    ...
                    let $t = $(lclSettings.dom.gallery_container+' table:first', $(this));
                    if ($t.length > 0) {
        ...
        /**
         * Updates album listings handlers.
         * @param {event} evt
         */
        ...
        bindGalleryHandlers: function( options ) {
            ...
            return this.each(function() {
                ...
                let imageDialogSettings = {
                    dialogClass: lclSettings.imageDialogClass,
                    formLoadCB: function() {
                        let $d = $(lclSettings.dialogContainer+':visible');
                        if ($d.length < 1) {
                            return;
                        }
        ...
        /**
         * Update gallery listings content.
         * @param {object} data
         */
        updateGalleryListingsContent: function( data ) {
            ...
            let gallery_selector = lclSettings.dom.gallery_container+'[data-pid='+data.parent_id+']';
            data.settings = {
        ...
        bindThumbnailEditButtons: function( evt ) {
            ...
            let $edit_btn = $('#edit-galltn-btn');
            let $del_btn = $('#del-galltn-btn');
        ...
        validateSlug: function( evt ) {
            ...
            $.post(
                ...
                function(data) {
                    if (data.content) {
                        /* slug input element */
                        let $i = $('input[name="'+lclSettings.edit.slug_param+'"]', $f);
                    ...
                    if (data.error) {
                        if (data.content) {
                            ...
                            let $e = $('.alert-error', $c);
                            if ($e.length < 1 ) {
                                let $e = $('<div class=\"alert alert-error hidden\"></div>');
                                $c.prepend($e);
                            }
        ...
        editImage: function( evt ) {
            ...
            let selector = methods.getListingsSelector.apply(this, evt.data);
        ...
        updateThumbnail: function( evt ) {
            ...
            let fd = $(this).formDialog('retrieveFormFilters', evt.data || {});
            ...
            let url = methods.getThumbnailURI(op, lclSettings);
            ...
            $.post(url, fd,
                function(data) {
                     ...
                     */
                    let dialogOptions = {formLoadCB: methods.displayThumbnailDialog};
                    if (op.indexOf('del')===0) {
        ...
        commitThumbnailUpdate: function(evt) {
            ...
            let lclSettings = $.extend(true, {}, settings, evt.data || {});
            let $f = $(lclSettings.dialogContainer+' form:first');

            /* collect the operation type */
            let op, $i = $('input[name=op]', $f);
            if ($i.length) {
                op = $i.val();
            }
            if (!op) {
        ...
        processThumbnailUpdate: function(data) {
            ...
            let lclSettings = $.extend(true, {}, settings, data.options || {});
        ...
        gotoAlbumPage: function(evt) {
            ...
            let extras = $(this).galleries('collectPageNavigationVariables', options);
            ...
            let fd = $(lclSettings.dom.album_filters).littled(
        ...
        updateAlbumContent: function(data) {
            ...
            let p = ((data.hasOwnProperty('page'))?(data.page):(1));
            if (!p) {
                p = 1;
            }

            $('.sm-process-wheel').removeClass('sm-active-wheel');
            let evt = $.Event('contentUpdate');
        ...
        gotoGalleryPage: function( evt ) {
            ...
            let $gallery = $(this).parents(lclSettings.dom.gallery_container+':first');
            let $f = $(lclSettings.dom.gallery_filters, $gallery);
            let extras = {};
            ...
            let filters = $.extend({}, extras, $('input, select', $f).serializeObject());
        ...
        bindFilterElementsHandlers: function( options ) {
            ...
            return this.each(function() {
                let $c = $('.gallery-page-nav:first', $(this));
        ...
        bindAlbumResorting: function ( options ) {
            ...
            return this.each(function() {
                ...
                let has_filtered_listings = false;
                let $f = $(lclSettings.dom.filters_form);
                if ($f.length > 0) {
                    let fd = $f.serializeObject();
        ...
        submitGalleryFilters: function( evt ) {
            ...
            let fd = $('input, select', $f).serializeObject();
        ...
        hasFilteredListings: function(fd) {
            let prop, exclude = $.extend({}, {p:'', pl: '', filt:'', ifilt:'', next: '', inext: '', ip: '', ipl: ''});
            for (prop in fd) {
