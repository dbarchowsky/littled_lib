		...
		if ($this->path->value) {
			if (isset($_REQUEST["MAX_FILE_SIZE"]) && $_REQUEST["MAX_FILE_SIZE"]>0) {
				if ($_SERVER["CONTENT_LENGTH"] > $_REQUEST["MAX_FILE_SIZE"]) {
					array_push($this->validationErrors,"The ".strtolower($this->path->label)." file is too large. Image file size is limited to ".$this->formatFileSize($_REQUEST["MAX_FILE_SIZE"]).".");
				}
			}
